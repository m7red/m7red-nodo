...
var NG;
NG = {
  version: '2015.06.27',            // current object version.
  debug: !1,                        // debug mode.
  ...
  d3_tip_offsetTop: 0,          // d3 tooltip top offset in pixels.
  d3_tip_offsetLeft: 0,         // d3 tooltip left offset in pixels.
  // Graph goes up here...
  run: function() {
    NG.isError = false;
  ...
  loadData: function() {
    d3.json(NG.dataURL, function(error, b) {
      ...
      if (NG.filterType && /^post|category|region$/.test(NG.filterType) &&
...
//       if (NG.isError) { return; }
  ...
  drawGraph: function() {
    ...
    var nodeOnMouseover = function(d, i) {
      ...
      if ((NG.filterType && /^category|region$/.test(NG.filterType)) &&
...
//       .offset([-25, 0])
      .offset([0, 0])
      .html(function(d) { return NG.nodes[d.id].excerpt; });
      NG.svg.call(tip);
...
//     console.log("zoomHandler()", d3.event.translate, d3.event.scale);
  ...
  setFilter: function() {
    ...
    // If all post entries should be shown we abort. There is no need for filter efforts.
