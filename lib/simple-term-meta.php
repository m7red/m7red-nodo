 ...
 *
 * Note: [Change]: 2015-apr-01
 *    This former plugin hasn't been updated for several years. It may no longer be
 *    maintained or supported and may have compatibility issues when used with more
 *    recent versions of WordPress.
 *    For this reason it is now used as part of normal theme library.
 *    A quick workaround to use the functionality furthermore w/o making
 *    many source code changes at present.
 *    Furthermore all function names have now a prefix to avoid overwriting.
 *
 *
...
m7red_simple_term_meta_install();

function m7red_simple_term_meta_install()
{
    // setup custom table
...
add_action( 'init', 'm7red_simple_post_meta_define_table' );

function m7red_simple_post_meta_define_table() {
    global $wpdb;
...
function m7red_simple_term_meta_uninstall()
{
    global $wpdb;
 ...
 */
function m7red_update_termmeta_cache($term_ids) {
 ...
 */
function m7red_add_term_meta( $term_id, $meta_key, $meta_value, $unique = false ) {
    return add_metadata('term', $term_id, $meta_key, $meta_value, $unique); // from WP Metadata API
}
 ...
 * @return bool False for failure. True for success.
 */
function m7red_delete_term_meta( $term_id, $meta_key, $meta_value = '' ) {
    return delete_metadata('term', $term_id, $meta_key, $meta_value); // from WP Metadata API
}
 ...
 *  is true.
 */
function m7red_get_term_meta( $term_id, $key, $single = false ) {
    return get_metadata('term', $term_id, $key, $single); // from WP Metadata API
}
 ...
 */
function m7red_update_term_meta( $term_id, $meta_key, $meta_value, $prev_value = '' ) {
    return update_metadata('term', $term_id, $meta_key, $meta_value, $prev_value); // from WP Metadata API
}
 ...
 */
function m7red_delete_term_meta_by_key($term_meta_key) {
    if ( !$term_meta_key )
        return false;
 ...
 * @return array
 */
function m7red_get_term_custom( $term_id ) {
    $term_id = (int) $term_id;

    if ( ! wp_cache_get($term_id, 'term_meta') )
        m7red_update_termmeta_cache($term_id);
 ...
 */
function m7red_get_term_custom_keys( $term_id ) {
    $custom = m7red_get_term_custom( $term_id );
 ...
 */
function m7red_get_term_custom_values( $key = '', $term_id ) {
    if ( !$key )
        return null;
